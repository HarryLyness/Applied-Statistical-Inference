...
```{r}
log_likelihood <- function(y, l1, l2, N = 1000) {
...
```{r}
nll_function <- function(y, lambda, N = 1000) {
  l1 <- lambda[1]
  l2 <- lambda[2]
...
for (i in 1:60){
  for (j in 1:60){
    M[i,j] = nll_function(lambda = c(l1_grid[i],l2_grid[j]),y = y_sample_q1,N=1000)
  }
}
```
...
fcn_log_likelihood_expr_param=function(theta , y , N=1000){
  total<-0 
  for (i in y){ 
...
grad_log_likelihood_expr_param <- function(theta, y, N = 1000) {
...
We notice that as N increases the values of lambda1 and lambda2 become less sensitive to change highlighting the need for large enough values of N as our MLE is sensitive to N. It is more sensitive to N = 10 and N = 100 
but less so at N = 1000 and after.
...
```{r}
hess_log_likelihood_expr_param <- function(theta, y, N = 1000) {
...
```{r}
fit_hessian=hess_neg_log_likelihood_expr_param(theta = MLE_theta, y=y_sample_q1,N=1000)
...
**KAI** **Im afraid this is incorrect, your std error is correct but the point estimate is theta1*exp(theta2) see below or the solutions for details**

```{r}
c(MLE_theta[1]*exp(MLE_theta[2])-1.96*sqrt(delta_hess),MLE_theta[1]*exp(MLE_theta[2])+1.96*sqrt(delta_hess))
```
...
```{r}
fit_hessian_lam = hess_neg_log_likelihood_expr(lambda = MLE_lambda, y = y_sample_q1, N = 1000) 
...
```{r}
fcn_log_likelihood_expr_param_H0=function(theta , y , N=1000){
  total<-0 
  for (i in y){ 
...
grad_log_likelihood_expr_param_H0 <- function(theta, y, N = 1000) {
...
likelihood_H0 <- function() {
  ...
  return(log_likelihood(y_sample_q1, MLE_lambda1, 5, 1000))
}

# Likelihood function under Ha
likelihood_Ha <- function() {
  return(log_likelihood(y_sample_q1, MLE_lambda[1], MLE_lambda[2], 1000))
}
...
neg_fcn_log_likelihood_expr_param_mu=function(theta , y , x , N=1000){
  total<-0 
...
neg_grad_log_likelihood_expr_param_mu=function(theta, y,x, N=1000){
...
mean <- rep(NA, n_grid)
ci <- matrix(NA, nrow = n_grid, ncol = 2)
H = neg_hess_log_likelihood_expr_mu(theta = optim_F1_mu_min$par, y = data_q7$y, x = data_q7$x, N = 1000) 
for (i in 1:n_grid) {
...
**KAI Your confidence intervals  for the F1 family are simply too wide! I could not find why, but the problem is in the asymptotic variance since the AIC are correct! Please have a look at the solutions for details**
...
ydat = data_q7$y
for (i in c(1:n)){
  temp = matrix(neg_grad_log_likelihood_expr_param_mu(theta = c(optim_F1_mu_min$par), x = xdat[i], y = ydat[i], N=1000))
...
**KAI** **Again! Confidence intervals for the F1 family are too wide! See solutions for details!**
